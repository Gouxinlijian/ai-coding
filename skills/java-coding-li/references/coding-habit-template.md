...
- 为减少主方法行数而制造大量只调用一次的碎片方法。
- 一个 Service 长期混合互不相关的业务、定时调度、外部适配和数据访问职责。
- 为形式补充作者日期、长篇 JavaDoc、逐行注释或没有处理计划的 TODO。
...
- 是否已经查看同包实现、调用方、已有 Repo、Converter、工具、常量、枚举和测试。
- 主流程、事务边界、锁、状态流转、外部动作、失败语义和兼容性是否明确。
- 是否真的需要新增类、接口、DTO、工具或设计模式。

写码后确认：

- 入口方法能否从上到下读懂，方法拆分是否对应完整能力。
- 命名、方法顺序、注释密度和 `if/for` 与 Stream 的选择是否符合本模板。
- 是否误改无关格式、日志、返回协议或存量结构。
- 是否需要新增或更新单元测试，并完成与风险相匹配的定向验证。
