...
- 编码 skill 源目录为 `skills/java-coding-guide/` 和 `skills/csharp-coding-guide/`。分别复制整个目录及全部子文件；两个 skill 都是公开 skill，不得替换其中任何文字、称呼或个人信息。
...
2. 将完整 `skills/java-coding-guide/` 与 `skills/csharp-coding-guide/` 分别安装到 `$CODEX_HOME/skills/java-coding-guide/` 和 `$CODEX_HOME/skills/csharp-coding-guide/`；`CODEX_HOME` 未设置时使用 `~/.codex/skills/`。
3. 校验目标全局规则除 `USER_TITLE` 替换外与源 `AGENTS.md` 一致；递归校验两个 skill 的每个文件 SHA-256 与源目录一致。

## Claude

1. 将处理过称呼替换的全局规则安装到 `~/.claude/CLAUDE.md`。
2. 将完整 `skills/java-coding-guide/` 与 `skills/csharp-coding-guide/` 分别安装到 `~/.claude/skills/java-coding-guide/` 和 `~/.claude/skills/csharp-coding-guide/`。
3. 校验目标全局规则除 `USER_TITLE` 替换外与源 `AGENTS.md` 一致；递归校验两个 skill 的每个文件 SHA-256 与源目录一致。
